'''Provides all HTML forms for user interaction'''
...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user is not None:
            raise ValidationError('Dit gebruikersnaam is al in gebruik, gebruik '
                                  'alstublieft een ander gebruikersnaam.')

    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user is not None:
            raise ValidationError('Dit email adres is al in gebruik, gebruik '
                                  'alstublieft een ander email adres.')
...
class ChecklisteForm(FlaskForm):
    list_of_files = ['Binnenland', 'Buitenland', 'Economie', 'Milieu',
                     'Wetenschap en technologie', 'Immigratie en integratie',
                     'Justitie en Criminaliteit', 'Sport',
                     'Kunst, cultuur en entertainment', 'Anders/Diversen']
...
class ReportForm(FlaskForm):
    lead = TextAreaField('Onderwerp:', validators=[DataRequired()])
    message = TextAreaField('Wat voor soort problemen zijn er met het artikel?',
